...
| identityKey | 独立内部 Opaque Secret `plugin-halo-weapp-identity` | 256 bit；只使用二进制 data；与 Setting ConfigMap 隔离；不进入表单、公开 DTO、日志、jar 或测试快照；加密备份 |
| 微信读者昵称 | WeAppUser | 用户主动提供；2～20 字；创建/修改均通过 msgSecCheck；可注销 |
| 读者身份摘要前缀 | WeAppUser metadata.name | 不返回客户端、不聚合到 anonymous；随账号注销删除；Halo core 索引日志可能显示该 HMAC 截断名，按可关联标识保护 |
...
5. **插件 ↔ WeAppUser/Secret 扩展存储**：WeAppUser scheme 不聚合 anonymous；资源不保存原始
   OpenID，identityKey 只通过内部 Secret 服务访问。
...
| T-09 | 日志泄露敏感信息 | OpenID/凭据外泄 | 插件日志只记 requestId、trace_id、suggest、label、耗时、HMAC(OpenID+salt)；大小上限；Halo core 可能记录 HMAC 资源名，日志访问受限 |
...
| T-14 | identityKey 泄露、丢失或被设置重置覆盖/静默轮换 | 可离线关联内部身份，或所有账号无法恢复 | 32 字节随机 key；独立 Opaque Secret.data；加密备份和恢复演练；已有 WeAppUser 时 Secret 缺失/空值直接失败关闭，不自动生成；禁止日志/表单/公开 DTO；轮换必须迁移 |
...
| T-23 | Halo 扩展 GC 删除日志序列化敏感 ConfigMap.data | identityKey 进入平台 INFO 日志 | 禁止用 ConfigMap 保存 key；Secret.data 删除日志只显示 byte[] 引用；早期 RC ConfigMap 在启动前原值迁移并清除，冲突/损坏阻止启动；运行时日志用实际 key 扫描 |
...
- v0.4.0 生产默认关闭 Moment 评论写入；本地 P1 契约已实现，但预留开关不得在外部门禁完成前启用。

## 5. 发布前检查

- [ ] 依赖漏洞扫描、凭据扫描（AppSecret/access token/session_key/OpenID 模式）通过；
- [ ] 使用实际凭据值扫描插件/Halo 日志：AppSecret、OpenID、token、session_key、identityKey 命中数为 0；
- [ ] 单元测试覆盖 review/risky/超时/未知 suggest 全部失败关闭路径；
- [ ] 频控触发时无微信/Halo 外部调用（mock 断言）。
- [ ] 同一 OpenID 并发首次登录只创建一个 WeAppUser，资源/错误/日志无原始 OpenID；
- [ ] identity Secret 损坏，或已有 WeAppUser 时 Secret 缺失/为空，均失败关闭且不生成 key；
- [ ] 早期 RC ConfigMap 原值迁移后已清除 key，Secret 指纹一致，迁移/删除日志无 key；
...
- [ ] v0.1.1 正式产物/tag 哈希一致；目标环境执行 v0.2.0 → v0.1.1 → v0.2.0 后
  Setting ConfigMap、identity Secret/WeAppUser、旧客户端与 Moment 均保持可用。
