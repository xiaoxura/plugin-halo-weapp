# plugin-halo-weapp v0.1.1：部署、升级与回滚

> 对应开发计划 §14「分阶段上线与回滚」。插件与小程序的版本组合须满足
> 兼容矩阵：`plugin-halo-weapp` v0.1.1 ↔ HaloWeApp v0.3.0，Halo `>= 2.23.0`。
> v0.1.0 的 Setting 资源布局、Spring 构造器选择和匿名 config RBAC 均不满足
> Halo 生产运行时要求，不得作为安装或回滚版本。
...
1. 安装 `build/libs/plugin-halo-weapp-0.1.1.jar`（Halo 后台 → 插件 → 安装）并启用；
2. 打开插件设置：
...
- v0.1.0：不要继续启用；直接安装 v0.1.1，并确认插件进入 `STARTED`、设置页可打开且
  匿名 `/config` 返回 200 JSON；
...
从 v0.2.0 回滚服务端时，二进制基线必须选择 v0.1.1，不得选择 v0.1.0；安装后至少验证
插件状态、Setting、匿名 `/config`、`/session` 业务错误以及文章评论路由。
v0.1.1 的 Setting 会显示一组明确标记为“不生效”的 v0.2.0 前向配置，仅用于在 Halo
协调过程中保留 ConfigMap；回滚期间所有这些开关仍须保持关闭。
